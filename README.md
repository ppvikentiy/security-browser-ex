...
> **Что нового в 2.0.0** — усилена защита внутреннего канала настроек: все сообщения между изолированным миром и MAIN‑миром страницы теперь подписываются **HMAC‑SHA256** с одноразовым ключом на загрузку страницы и защитой от повторов (replay). Чувствительная конфигурация анти‑фингерпринта больше не сохраняется в `localStorage` страницы; критичные модули стартуют **fail-closed**. Подробности — в разделе [«Защита внутреннего канала»](#защита-внутреннего-канала-hmac).
...
Кэш в `localStorage` (`__focus_blocker_*_cache_v1`) используется как запасной путь до прихода настроек с моста только для **не‑секретных флагов** (focus, ADS Block, Threat Shield). Полная конфигурация анти‑фингерпринта в `localStorage` страницы **не сохраняется**, а Network Security и Device Security не понижают свой статус (`isActive`) на основании кэша — только по подписанному сообщению с моста.
...
* **`src/fb-channel.js`** — общая библиотека (подключается первой в каждой записи `content_scripts`): чистый JS SHA‑256 / HMAC‑SHA256 (синхронный, работает и на `http://`, где недоступен `crypto.subtle`) и детерминированная сериализация `stableStringify` с сортировкой ключей. Все используемые нативы (`TextEncoder`, `JSON.stringify`, `Object.keys`, `Array.prototype.sort`, `Uint8Array` и т.д.) захватываются на `document_start` до выполнения скриптов страницы, а экспортируемый API (`globalThis.__fbChannel`) заморожен — страница не может подменить методы и подсмотреть ключ
* **Ключ** — 32 случайных байта на каждую загрузку страницы; мост передаёт его MAIN‑модулям через короткоживущий атрибут `<html data-fb-k="...">`, который снимается после первой рассылки. Ключ **никогда не попадает в сами сообщения**
...
* **Fail-closed** — Network Security и Device Security стартуют включёнными и могут быть отключены только подписанным сообщением; `workerScriptUrl` для анти‑фингерпринта принимается только с `chrome-extension://`‑URL собственного расширения; критичные перехваты API закреплены через `configurable: false`
* Обработчик косметического CSS в `background.js` дополнительно проверяет `sender.id` и ограничивает размер CSS

Остаточный риск: атрибут с ключом существует от `document_start` до первой рассылки настроек (миллисекунды) — inline‑скрипт в самом начале `<head>` теоретически может успеть его прочитать. Планка атаки поднята с «пассивно слушать `postMessage`» до «активно читать DOM‑атрибут в узком окне загрузки».
...
* `src/fb-channel.js` — криптопримитивы канала настроек (HMAC‑SHA256, каноническая сериализация)
