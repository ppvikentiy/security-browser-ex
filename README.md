...
> **Что нового в 2.5.0** — исправлена критическая ошибка доставки настроек в MAIN‑мир страницы (на vk.ru и других сайтах): один и тот же путь скрипта, перечисленный в нескольких записях `content_scripts`, внедрялся в документ только один раз — изолированная запись забирала единственное внедрение, и MAIN‑модули не получали подписанный канал HMAC. В результате Threat Shield молчал, Device Security оставался fail‑closed и глушил IndexedDB, статистика из MAIN не считалась. Добавлены отдельные байт‑идентичные копии для MAIN‑мира — `fb-channel-main.js` и `security-defaults-main.js`; все MAIN‑скрипты объединены в одну запись манифеста. Для проверки синхронизации копий: `node tools/check-world-copies.mjs [--fix]`. Подробности — в разделе [«Защита внутреннего канала»](#защита-внутреннего-канала-hmac).
