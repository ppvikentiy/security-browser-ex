...
> **Что нового в 2.7.0**
>
> * **Мультиязычный интерфейс** — русский, English и українська. Язык выбирается в разделе «Доступность» (`optionsUiLanguage`) и сразу применяется к странице параметров, всплывающему окну и баннеру Threat Shield («Активная интернет защита»). Каталог строк — [`src/i18n.js`](./src/i18n.js) (генерация: `node tools/gen-i18n.mjs`); разметка UI помечена атрибутами `data-i18n*`.
> * **Иконка** — обновлённый замок и PNG‑ассеты в [`assets/`](./assets/).
> * **Пауза вкладки** — исправлена доставка состояния в content scripts: корректный слушатель `chrome.storage.session.onChanged` и явное сообщение `FB_REFRESH_SETTINGS` после pause/unpause (и при сбросе паузы при смене URL). Раньше модули могли продолжать работать после «Временно отключить на этой вкладке».
> * **Помощник при копировании** — учитывает глобальное выключение расширения и паузу вкладки (раньше подсветка оставалась активной).
> * **Страница параметров** — при сохранении больше не теряются правки, если предыдущая запись в `storage` ещё не завершилась (очередь повторного flush).
