...
Chromium extension (Manifest V3): focus/visibility hardening, anti-fingerprinting, optional network and device privacy tools, **on-page caution banners** when heuristic checks trigger, statistics, and a full options UI.
...
* Hooks `addEventListener` / `removeEventListener` / `dispatchEvent` for those types and inline handlers on `window` / `document`
...
* **Privacy pack** — separate DNR tracker list (narrow vs wide resource types); independent of DS cosmetic/popup logic
...
* **Statistics** — per top-level host counters; with `declarativeNetRequestFeedback`, DNR matches feed the badge/table
...
| Permission | Role |
|------------|------|
| `storage` | Options in `local`/`sync`; session maps (`paused` tabs, cosmetic CSS text, per-tab stat bucket) |
| `tabs`, `windows` | Popup state, badge, hostname for stats |
| `scripting` | Insert/remove DS cosmetic CSS |
...
4. **DS telemetry** — chunked `requestDomains` rules from slot `990060`
...
Also: tab pause map `focusBlockerPausedTabIds`, DS cosmetic CSS per tab, stats key `focusBlockerStatsByHost`, badge from `focusBlockerTabStat` (session).
...
* `src/ds-block.js` — popups/telemetry/cosmetic coordination
